...
const regions = ['제주도', '서울', '부산', '강릉', '경주', '여수']

export function Planner({ onCreated }: { onCreated: (url: string) => void }) {
  const today = new Date().toISOString().slice(0, 10)
  const [draft, setDraft] = useState<TripDraft>({ people: 2, region: '', startDate: today, endDate: '' })
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')

  async function submit(event: FormEvent) {
    event.preventDefault()
    if (!draft.region || !draft.startDate || !draft.endDate) { setError('지역과 여행 기간을 모두 선택해 주세요.'); return }
  ...
  return <section className="planner-section" id="planner"><div className="planner-copy"><p className="eyebrow">YOUR JOURNEY, YOUR WAY</p><h2>여행을 계획해 볼까요?</h2><p>인원, 지역, 기간만 알려주시면 나머지는 TripTailor가 맞춰 드릴게요.</p></div><form className="planner-bar" onSubmit={submit}>
    ...
    <label className="planner-field"><span className="field-icon">⌖</span><span><small>지역</small><select value={draft.region} onChange={(e) => setDraft({ ...draft, region: e.target.value })}><option value="">가고 싶은 지역</option>{regions.map(region => <option key={region}>{region}</option>)}</select></span></label>
