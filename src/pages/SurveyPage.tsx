...
export function SurveyPage({ inviteCode }: { inviteCode: string }) {
  ...
  useEffect(() => {
    getInvitation(inviteCode)
      .then(setInvitation)
      .catch(cause => setError(cause instanceof Error ? cause.message : '초대 정보를 불러오지 못했습니다.'))
  }, [inviteCode])
  ...
  if (!invitation) return <main className="simple-page"><Header /><section className="center-card"><p>{error || '초대 정보를 불러오는 중…'}</p></section></main>
  ...
  return <main className="survey-page"><Header /><form className="survey" onSubmit={submit}><section className="invite-intro"><p>여행 참여가 완료되었어요</p><h1><Brand /></h1><p>{invitation.regionName} 여행을 더 잘 맞추기 위해 몇 가지만 알려주세요.</p><span className="people-pill">최대 {invitation.participantLimit}명</span></section><SurveyInput title="이름 또는 닉네임" name="displayName" required placeholder="예: 지민" /><SurveyInput title="참여 가능한 날짜가 있나요?" name="availableDateText" placeholder="예: 9월 셋째 주 주말, 추석 연휴 등" /><ChoiceCard title="1인 예산은 어느 정도가 좋을까요?" name="budgetBand" choices={budgetChoices} /><ChoiceCard title="선호하는 숙소는?" name="accommodationType" choices={accommodationChoices} /><SurveyInput title="꼭 가고 싶은 곳이나 하고 싶은 게 있나요?" name="mustHaves" placeholder="가고 싶은 장소, 먹고 싶은 음식 등 자유롭게 적어주세요" /><SurveyInput title="그 외 하고 싶은 말" name="additionalNotes" placeholder="알레르기, 이동 제약, 기타 요청사항 등" />{error && <p className="form-error" role="alert">{error}</p>}<button className="primary-button submit-button" disabled={submitting}>{submitting ? '제출 중…' : '제출하기'}</button></form></main>
}
